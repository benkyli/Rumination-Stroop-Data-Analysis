expname="Emotion_Standard_Stroop"
rtcol = 7
successcol = 6
blockcol = 1
trimSD = NA
...
for (i in 1:length(d[, 1])) {
    if (!is.na(d[i, 1]) & file.info(d[i, 1])$size > 0) {
        ...
        for (i in 1:length(conditionnames)) {
            ...
            ## Add SD
            if (length(outputdataSD) == 0) {
                outputdataSD = sd(tmpRtData)
            } else {
...
colnames(outputSD) = conditionnames ## Add SD
...
outputSD2     = matrix(ncol=length( conditionnames ),nrow=length(d[,1])) ## Add SD
...
colnames(outputSD2)     = conditionnames ## Add SD
...
for( i in 1:length(d[,1])){
    if( is.na(d[i,1]) | file.info(d[i,1])$size == 0 ){
        ...
        outputSD2[i,] = rep( NA , length( conditionnames ) ) ## Add SD        
    }else{
        ...
        outputSD2[i,]     = outputSD[counter,] ## Add SD
        counter=counter+1
    }
}
...
write.csv(outputSD2, "Emotion_Standard_Stroop.sd.csv", row.names = F)
